...
const Cell = ({ value, current, onChange }: CalendarCellProps) => {
  ...
  if (value.type === CalendarItemType.Week) {
    ...
    // }
    return (
      <td key={`week-${value}`} className={classes.join(' ')} onClick={handleChange}>
        {itemDate.week()}
      </td>
    );
  } else {
...
const Week = ({
  ...
  return (
    <tr className={weekClasses.join(' ')}>
      {weekRange.map((d) => (
        <Cell key={d.toString()} value={d} current={current} onChange={onChange} />
      ))}
    </tr>
  );
};
...
export const Calendar = ({ current, onChange }: CalendarViewProps) => {
  ...
  const shiftMonth = (diff: number) =>
    useCallback(() => {
      onChange(
        new CalendarItem(moment(currentDate).startOf('month').add(diff, 'month'), CalendarItemType.Month),
        false
      );
  ...
  const yearClasses = [
    /*"chronology-calendar-selectable"*/
  ];
  ...
  return (
    <div className="chronology-calendar-box">
      <table className="chronology-calendar-grid">
        <thead>
          ...
          <tr>
            {daysOfTheWeek.map((dow) => (
              <th className="chronology-grid-dayofweek" key={dow}>
                {dow ||
                  (!isToday && (
                    <span
                      className="chronology-calendar-todaylink chronology-calendar-selectable"
                      title="today"
                      onClick={selectToday}
                    >
                      ⏎
        ...
        <tbody>
          {monthRange.map((week) => (
            <Week key={week[1]} week={week} current={current} onChange={handleChange} />
          ))}
        </tbody>
      </table>
    </div>
